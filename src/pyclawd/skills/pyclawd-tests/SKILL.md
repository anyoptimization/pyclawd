...
**`golden` is a third, separate marker — keep it out of the unit suite.** Behavior-regression snapshot tests (`@pytest.mark.golden`, see `pyclawd-golden`) answer "did a number move?", not "is this logic correct?", and need committed baselines. Exclude them from every unit tier (`"fast": "not slow and not integration and not golden"`, `"default": "not slow and not golden"`, `"all": "not golden"`) and run them as their own gate with `pyclawd golden` (or `pytest -m golden`) — not via `pyclawd test`.
